...
scripts/fetch-models.sh z-image-turbo          # ~21 GB; makes the frontend's own default workflow run as-is
```

Closing every workflow tab opens the frontend's built-in default, which is currently a Z-Image-Turbo graph
(`z_image_turbo_bf16` + `qwen_3_4b` + `ae`). That is why an empty workspace showed red missing-model and missing-node
errors instead of a runnable canvas: the models were never downloaded and `ModelSamplingAuraFlow` was not allowlisted.
Both are fixed above, so the default runs. The class starters (Flux image first, Wan video, gated H3) are unchanged.
